...
class AuctionTest @Autowired constructor(
	...
	@Test
	fun `경매 가격 제시 - 3번`() {
		postService.bid(1L, 3L, 31000, Instant.now())
		postService.bid(7L, 3L, 30500, Instant.now())
		postService.bid(9L, 3L, 50000, Instant.now())
		println(postService.bidList(3L))
	}

	@Test
	fun `최소 가격보다 낮은 가격 제시`() {
		postService.bid(9L, 3L, 20000, Instant.now())
	}
	...
	fun `같은 가격 제시할 때 선착순으로 랭킹`() {
		...
		postService.bid(9L, 3L, 50000, Instant.now())
		postService.bid(7L, 3L, 30500, Instant.now())
		postService.bid(10L, 3L, 50000, Instant.now().plusMillis(10000))
		println(postService.bidList(3L))
	}

	@Test
	fun `가격 업데이트`() {
		postService.bid(1L, 3L, 51000, Instant.now())
		println(postService.bidList(3L))
	}
